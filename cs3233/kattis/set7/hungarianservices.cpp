...
public:
  ...
  void add_edge(int u, int v, ll w, ll c, bool directed = true) {
    ...
    AL[u].push_back(sz(EL)-1);                // remember this index
    EL.emplace_back(u, 0, 0, -c);                // back edge
    AL[v].push_back(sz(EL)-1);                // remember this index
...
int main() {
  ...
  cin.tie(nullptr);
  int Vleft, Vright, E; cin >> Vleft >> Vright >> E;
  min_cost_max_flow mf(Vleft + Vright + 2);
  int source = 0, sink = Vleft + Vright + 1;
  rep(i, 0, Vleft) {
    mf.add_edge(source, i + 1, 1, 0);
  }
  rep(i, 0, Vright) {
    mf.add_edge(i + 1 + Vleft, sink, 1, 0);
  }

  for (int i = 0; i < E; ++i) {
    int u, v, c; cin >> u >> v >> c;
    v += Vleft;
    mf.add_edge(u, v, 1, -c);                      // default: directed edge
  }
  pair<ll, ll> res = mf.mcmf(source, sink);
  // max flow, then the min cost possible for that max flow
  cout << Vleft - res.first << " " << -res.second << "\n";
  mf.print_allocs();
  return 0;
}