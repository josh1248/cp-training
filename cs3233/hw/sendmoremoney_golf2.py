...
for a in t.combinations('0123456789',len(l)):
 for p in t.permutations(a):
  m=str.maketrans(l, p:=''.join(p))
  if all(p[i]!='0'for i in z)and((i:=int)(p[c[0]])+i(p[c[1]])-i(p[c[2]]))%10+1 and(R==0 or p<b)and(x:=[i(n.translate(m))for n in s])[0]+x[1]==x[2]:R,b=x,p
