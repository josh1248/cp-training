...
int main() {
    ...
    for (int i = 0; i < E; i++) {
        int u, v, w;
        cin >> u >> v >> w;
        // undirected - indicate reverse flow possible
        mf.addEdge(u, v, w, w); 
    }

    // find mincut == min edges between S and T
    // add 1 to all these edges
    mf.calc(S, T);
    vector<pi> ans;
    for (int i = 1; i <= V; i++) {
        // add 1 to each edge of min cut
        for (auto& [j, rev, cap, flow]: mf.adj[i]) {
            if (mf.leftOfMinCut(i) && !mf.leftOfMinCut(j))
