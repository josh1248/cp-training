...
int main() {
    ...
    vector<tuple<int, int, ll>> originals;
    ...
    ll mf_res = mf.maxflow_edmondskarp(s, t);

    vector<tuple<int, int, ll>> res;
    for (auto& [u, v, cap]: originals) {
        ll diminished_cap = mf.AL[u][v];
        if (cap > diminished_cap) {
            res.emplace_back(u, v, cap - diminished_cap);
        }
    }

    cout << n << " " << mf_res << " " << sz(res) << "\n";
    for (auto& [u, v, w]: res) {
        cout << u << " " << v << " " << w << "\n";
    }
}